...
\begin{tikzpicture}[
	...
	% DNS Server
	\node[box, above=2cm of host] (dns) {DNS Server\\8.8.8.8};

	% Arrows connecting
	\draw[->] (host) -- node[net, above] {Virtual Network} (guest);
	\draw[->] (host) -- node[net, left] {DHCP Request} (dhcp);
	\draw[->] (guest) -- node[net, right] {DHCP Response} (dhcp);
	\draw[->] (guest) -- node[net, below right] {Internet Access} (host);
	\draw[->] (host) -- node[net, above] {DNS Server} (dns);
\end{tikzpicture}
